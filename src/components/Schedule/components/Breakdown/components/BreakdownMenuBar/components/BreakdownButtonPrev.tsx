...
import { getDNDContainerIdByChildId, getPreviousItemInArray } from 'helpers'

export function BreakdownButtonPrev(): JSX.Element {
  ...
  const handleClick = useCallback((): void => {
    const itemId = sch_breakdown_selectedIds[0]
    const boardId = getDNDContainerIdByChildId({ dndItems: temp_boards, itemId })
    if(!boardId) return
    const id = getPreviousItemInArray({ arr: temp_boards[boardId] as string[], currentItem: itemId as string })
    if(!id) return
    dispatch(setSch_breakdown_selectedIds([id]))
  },[dispatch, sch_breakdown_selectedIds, temp_boards])

  return (
    <Tip text={t('Prev Breakdown')}>
      <Nav.Item as={Button} size='sm' variant='info' onClick={handleClick}><FaArrowLeft /></Nav.Item>
    </Tip>
  )
}
