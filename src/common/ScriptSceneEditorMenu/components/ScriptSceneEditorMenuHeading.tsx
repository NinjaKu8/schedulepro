...
export function ScriptSceneEditorMenuHeading({ scriptSceneId }: Props): JSX.Element {
  ...
  const active = useIsProseMirrorNodeActive('heading')
  const onClick = useProseMirrorToggleNode('heading')
  return (
    <ScriptSceneEditorMenuNavItem active={active} callback={onClick} disabled={!isScriptEdit || isOmit} name='Heading' />
  )
}