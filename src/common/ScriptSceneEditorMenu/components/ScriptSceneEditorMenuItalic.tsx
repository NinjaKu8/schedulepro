...
export function ScriptSceneEditorMenuItalic({ scriptSceneId }: Props): JSX.Element {
  ...
  const active = useIsProseMirrorMarkActive('em')
  const onClick = useProseMirrorToggleMark('em')

  return (
    <ScriptSceneEditorMenuNavItem active={active} callback={onClick} disabled={!isScriptEdit || isOmit} name={<BsTypeItalic />} value='italic' />
  )
}