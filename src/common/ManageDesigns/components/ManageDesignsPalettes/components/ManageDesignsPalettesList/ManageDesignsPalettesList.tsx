...
import { ManageDesignsPalettesListData, ManageDesignsPalette } from "./components"
import { IDNDData, DNDItem } from "types/types"

export function ManageDesignsPalettesList(): JSX.Element {
  const { t } = useTranslation()
  return (
    <ManagerList>
      <ManageDesignsPalettesListData containerId='managedesignspalettes'>
          ...
          <>
            ...
            <div className='overflow-auto flex-grow-1 rounded-1'>
              <DNDContainer 
                ...
                overlayComponent={draggingId && <ManageDesignsPalette id={draggingId} isSelected={isSelected(draggingId)} />}
              ...
              >
                <DNDItems
                  ...
                  handle={false}
                  id='managedesignspalettes'
                  items={filteredItems}
                  onSelect={onSelect}
                >
                  {(id: DNDItem)=>( <ManageDesignsPalette id={id} isSelected={isSelected(id)} /> )}
                </DNDItems>
              </DNDContainer>
            </div>
